...
get '/hello' do
    ...
    msgUrl = uri + '/message/2064462459'
    getmsgURL = URI.parse(msgUrl)
    getmsgReq = Net::HTTP::Put.new(getmsgURL.request_uri)
    getmsgReq['Authorization'] = 'Bearer ' + token
    getmsgHttp = Net::HTTP.new(getmsgURL.host, getmsgURL.port)
    getmsgHttp.use_ssl = true
    
    msgStatus = getmsgHttp.request(getmsgReq)
    
    
    puts 'Send messages2'
    
    msgUrl = uri + '/message/2067657889'
    getmsgURL = URI.parse(msgUrl)
    getmsgReq = Net::HTTP::Put.new(getmsgURL.request_uri)
    getmsgReq['Authorization'] = 'Bearer ' + token
    getmsgHttp = Net::HTTP.new(getmsgURL.host, getmsgURL.port)
